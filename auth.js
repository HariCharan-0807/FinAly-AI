...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (savedView === 'mfa-setup-form' && savedToken && savedUri) {
    ...
    renderQrCode(savedUri, sessionStorage.getItem('mfa_secret') || '');
...
// ═══════════════════════════════════════════════════════════
function renderQrCode(uri, secret) {
  const qrContainer    = document.getElementById('qr-container');
  const secretDisplay  = document.getElementById('totp-secret-display');

  if (!qrContainer) return;

  // Embed via QR Server API — displayed as inline image, NOT a link
  const encoded = encodeURIComponent(uri);
  const imgSrc  = `https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encoded}&bgcolor=ffffff&color=000000&format=png`;
...
async function triggerMFASetup(token, email) {
  try {
    ...
    renderQrCode(data.totp_uri, data.secret);
